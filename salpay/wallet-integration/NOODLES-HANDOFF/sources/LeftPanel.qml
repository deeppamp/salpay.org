...
Rectangle {
    ...
    // Expose asset dropdown expanded state so main.qml can avoid rebuild flicker.
    readonly property bool assetDropdownExpanded: assetTypeSelector.expanded
    ...
    function setAssetTypes(list) {
        ...
        for (var i = 0; i < list.length; ++i)
            assetTypes.append({ column1: String(list[i]) });

        // Restore selection by asset name (e.g. "SAL1", "DEEP") — not by numeric index.
        var desired = String(appWindow.persistentSettings.assetType || "SAL1");
        var idx = findIndexByValue(desired);
        if (idx < 0) {
            idx = 0;
            if (assetTypes.count > 0)
                appWindow.persistentSettings.assetType = assetTypes.get(0).column1;
        }
    ...
    function dispatchAction(action) {
        ...
        case "SalPay": panel.salpayClicked(); break;
        default: break;
        }
    }
    ...
    function selectItem(pos) {
        ...
        else if (pos === "SalPay") selectButton(salpayButton)
    ...
    ColumnLayout {
        ...
        Item {
            ...
            Flickable {
                ...
                anchors.right: parent.right
                anchors.top: parent.top
                anchors.bottom: bottomStatusColumn.top
                clip: true
                // Windows often needs explicit vertical flick + scrollbar; otherwise
                // Advanced/Settings are only reachable when the window is tall enough.
                interactive: true
                flickableDirection: Flickable.VerticalFlick
                boundsBehavior: isMac ? Flickable.DragAndOvershootBounds : Flickable.StopAtBounds
                contentWidth: width
                contentHeight: menuColumn.childrenRect.height + 16
                ...
                Column {
                    id: menuColumn
                    width: menuFlick.width - menuScrollBar.width - 4
                    anchors.left: parent.left
                    ...
                    MoneroComponents.MenuButtonDivider { visible: salpayButton.present; anchors.left: parent.left; anchors.right: parent.right; anchors.leftMargin: 20 }
                    ...
                    // Settings before Advanced so it stays on-screen without scrolling
                    // past Create Token / Staking / etc. on short windows.
                    MoneroComponents.MenuButton {
                        id: settingsButton
                        anchors.left: parent.left
                        anchors.right: parent.right
            ...
            // Bottom status stack — fixed heights so bars stay fully on-screen
            // inside the 300px left panel (no horizontal overflow / cut-off).
            Column {
                id: bottomStatusColumn
                anchors.left: parent.left
                anchors.right: parent.right
                anchors.bottom: parent.bottom
